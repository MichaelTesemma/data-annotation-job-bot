...
from categories import all_search_terms
...
class Settings:
    ...
    translation_search_terms: list[str] = field(default_factory=list)
    rate_limit_seconds: float = 2.0

    @property
    def all_search_terms(self) -> list[str]:
        # Single source of truth: every category's search terms. Local
        # overrides in config.local.py merge in on top.
        terms = list(all_search_terms())
        for extra in (*self.search_terms, *self.translation_search_terms):
